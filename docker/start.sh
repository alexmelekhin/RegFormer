...
docker run -it -d --rm \
    $ARGS \
    --privileged \
    --name ${USER}_regformer \
    --net host \
    -v $PROJECT_ROOT_DIR:/home/docker_regformer/RegFormer:rw \
    -v $DATASETS_DIR:/home/docker_regformer/Datasets:rw \
    regformer:$DEVICE-$USER

docker exec --user root \
    ${USER}_regformer bash -c "/etc/init.d/ssh start"
